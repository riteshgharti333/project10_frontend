...
const EditLedger = () => {
  ...
  // Format the ledger name for display
  const formattedLedgerName = ledgerName 
    ? ledgerName.charAt(0).toUpperCase() + ledgerName.slice(1).replace("-", " ")
    : "";

  // Get the corresponding form component based on ledgerName
  const getLedgerForm = () => {
    switch(ledgerName) {
      case 'patient-ledger':
        return <PatientLedgerForm />;
      case 'doctor-ledger':
        return <DoctorLedgerForm />;
      case 'supplier-ledger':
        return <SupplierLedgerForm />;
      case 'pharmacy-ledger':
        return <PharmacyLedgerForm />;
      case 'diagnostics-ledger':
        return <LabLedgerForm />;
      case 'cash-ledger':
        return <CashLedgerForm />;
      case 'bank-ledger':
        return <BankLedgerForm />;
      case 'insurance-ledger':
        return <InsuranceLedgerForm />;
      case 'general-ledger':
        return <ExpenseLedgerForm />;
      default:
        return <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 text-center">
          <p className="text-gray-600">Invalid ledger type specified</p>
        </div>;
    }
  };
  ...
  return (
    <div className="mx-auto">
      ...
      {/* Render the appropriate ledger form */}
      {getLedgerForm()}
    </div>
  );
};

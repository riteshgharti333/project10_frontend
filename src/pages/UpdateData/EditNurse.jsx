...
const EditNurse = () => {
  ...
  const onSubmit = async (formData) => {
    try {
      const response = await updateNurse({ id, data: formData });
  ...
  const handleDelete = async () => {
    try {
      const { data } = await deleteNurse(id);
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <h2 className="text-2xl font-bold text-gray-800 flex items-center ml-2">
              ...
              {editMode ? "Edit Nurse" : "View Nurse"}
            </h2>
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {section.fields.map((field, fieldIndex) => {
                ...
                const fieldValue = nurseData[field.name];
